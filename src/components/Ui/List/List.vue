...
const {image,text,title,styles} = defineProps({
    image:String,
    title:String,
    text:String,
    styles:String
...
<div :class="` my-3 w-11/12 flex items-center justify-around p-2 flex-wrap rounded-lg shadow-gray-800 shadow-lg ${styles}`">
      <div class="max-w-[500px] h-[480px] rounded-md">
      ...
      <div class="max-w-[600px] text-center">
        <h3 class="text-2xl text-start font-bold text-gray-50">{{ title }}</h3>
        <p class="text-xl text-start text-gray-50">{{ text }}</p>
      </div>
</div>
</template>